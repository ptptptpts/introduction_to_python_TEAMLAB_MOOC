...
def delete_a_list_element(list_data, element_value):
    ...
    if list_data.count(element_value) > 0:
...
def comparison_list_size(list_data_a, list_data_b):
    ...
    result = None
    if len(list_data_b) > len(list_data_a) :
        result = list_data_b
    else:
        result = list_data_a
    # ==================================
...
def number_of_cases(list_data):
    ...
    for ch1 in list_data :
        ch1 = str(ch1)
        for ch2 in list_data :
            ch2 = ch1 + str(ch2)
            if result.count(ch2) == 0 :
                result.append(ch2)
